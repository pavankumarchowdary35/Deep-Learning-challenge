...
from data import ChristmasImages # This points to your submitted code
...
def evaluate():
    path = 'data/data/val' ######### PROVIDE THE TEST DATASET ###########                                                  
    model = Network()
    model.eval()

    ############# PLEASE_NOTE #############
    ### WE HAVE USED OUR load_model function which is defined in model.py file
    ### As it is giving some problem related to GPU
    ### Instead of using GPU we have used CPU everywhere in this file

    # model.load_model('submission/model_final.pkl') 
    ...
    accuracy = TestSet.evaluate(model, loader, device='cpu' ) # torch.device('cuda:0')        
...
class TestSet(Dataset):
    ...
    def __init__(self, path):
        ...
        with open('submission/test_predictions_new.csv') as file: ############ PROVIDE THE TEST SET .CSV FILE ###############
